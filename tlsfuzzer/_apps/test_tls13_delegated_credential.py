...
def help_msg():
    ...
    print('                "secp256r1"')
    print(" --dc-sig-alg   expected signature algorithm to be used by server")
...
def main():
    ...
    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:e:x:X:n:C:g:", ["help", "dc-sig-alg="])
    for opt, arg in opts:
        if opt == "-h":
            host = arg
        elif opt == "-p":
            ...
            dc_sig_alg = getattr(SignatureScheme, arg)
        elif opt == "--help":
            help_msg()
            sys.exit(0)
        else:
    ...
    if dc_sig_alg in forbidden_sig_algs:
        raise ValueError(
            "The following algorithms are not allowed "
            "for use with delegated credentials: rsa_pss_rsae_sha256, "
    ...
    ext[ExtensionType.supported_versions] = SupportedVersionsExtension().create(
        [TLS_1_3_DRAFT, (3, 3)]
    )
    ...
    dc_ext = [dc_sig_alg]
    ext[ExtensionType.delegated_credential] = DelegatedCredentialExtension().create(
        dc_ext
    )
    node = node.add_child(
        ClientHelloGenerator(
            ciphers + [CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV], extensions=ext
        )
    )
    ...
    node = node.add_child(ExpectCertificateVerify(sig_alg=dc_sig_alg))
    ...
    node = node.add_child(
        ExpectAlert(
            level=AlertLevel.fatal, description=AlertDescription.handshake_failure
        )
    )
    node.next_sibling = ExpectClose()
    # if the server has an ability to use ceritificate's private key
    node.next_sibling.next_sibling = ExpectServerHello()
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["missing support of DC in client hello"] = conversation
    ...
    forbidden_alg = [SignatureScheme.rsa_pss_rsae_sha256]
    ext[ExtensionType.delegated_credential] = DelegatedCredentialExtension().create(
        forbidden_alg
    )
    node = node.add_child(
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["no mutually acceptable algorithm"] = conversation
    ...
    node = node.add_child(ExpectAlert())
    node.next_sibling = ExpectClose()
    conversations["empty"] = conversation
    ...
    print(
        "Communication test with TLS 1.3 server with support of delegated credentials."
    )
